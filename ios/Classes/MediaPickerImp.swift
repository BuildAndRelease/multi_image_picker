...
//                fetchResult不能倒序，所以在for循环的时候倒序
        ...
        case "pickImages":
            ...
            BSImagePickerViewController.authorize(fromViewController: ImpUtils.getTopViewController()!) { (authorized) -> Void in
                ...
                ImpUtils.getTopViewController()?.present(imagePicker, animated: true, completion: nil)
            }
            break;
    ...
    private class func cachedFilePath(url : URL?) -> String {
        if let _ = url {
    ...
    private class func getThumbnailSize(originSize: CGSize) -> CGSize {
